...
export function registerDatabase(program: Command): void {
  ...
  db
    ...
    .action(async (deployment, opts) => {
      try {
        ...
        printTable(
          ['SERVICE ID', 'TYPE', 'NAME', 'DEPLOYMENT', 'STATUS', 'AUTO-BACKUP', 'RETENTION'],
          services.map((s: any) => [
            s.id,
            s.serviceType || '—',
            ...
            s.backupEnabled ? `${s.backupRetentionDays ?? 7}d` : '—',
          ])
        );
      } catch (err) {
  ...
  // backup-upload (migrate an existing database in)
  db
    .command('backup-upload <service-id> <local-file>')
    .description('Upload a backup file from elsewhere so it can be restored into a NEXUS AI service')
    .option('--json', 'Output raw JSON')
    .action(async (serviceId, localFile, opts) => {
      if (!fs.existsSync(localFile)) {
        errorMsg(`File not found: ${localFile}`);
        process.exit(1);
      }
      const stat = fs.statSync(localFile);
      const spin = spinner(`Uploading ${formatBytes(stat.size)}...`);
      try {
        const stream = fs.createReadStream(localFile);
        const res = await client.post(
          `/api/deployment-services/${serviceId}/backups/upload`,
          stream,
        ...
        spin.stop();
        if (opts.json) { printJson(backup); return; }
        success(`Uploaded ${formatBytes(stat.size)} as backup ${backup.id}.`);
        console.log(`Run \`nexus db restore ${serviceId} ${backup.id}\` to restore it.`);
      } catch (err) {
        spin.stop();
        errorMsg(apiError(err));
        process.exit(1);
  ...
  db
    ...
    .option('--retention <days>', 'Days to keep auto-created backups (default 7, cannot go lower)')
    .option('--json', 'Output raw JSON')
    .action(async (serviceId, opts) => {
      ...
      const body: { enabled: boolean; retentionDays?: number } = { enabled };
      if (opts.retention !== undefined) {
        const retentionDays = parseInt(opts.retention, 10);
        if (!Number.isFinite(retentionDays)) {
          errorMsg('--retention must be a number.');
          process.exit(1);
        }
      ...
      try {
        const res = await client.patch(`/api/deployment-services/${serviceId}/backup/schedule`, body);
        const result = unwrap(res.data);
        if (opts.json) { printJson(result); return; }
        success(enabled ? 'Daily backups enabled.' : 'Daily backups disabled.');
        if (body.retentionDays !== undefined) {
          console.log(`Retention set to ${body.retentionDays} day(s).`);
        }
      } catch (err) {
        errorMsg(apiError(err));
        process.exit(1);
